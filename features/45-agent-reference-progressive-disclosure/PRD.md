...
> restructure `AI-AGENT-REFERENCE.md` *before* Feature 44 measures anything, so
> the eval measures the artifact we intend to ship rather than one we are about
> to replace.
>
...
`AI-AGENT-REFERENCE.md` is 27,489 bytes. Section sizes, measured with code
...
| Task shape | Needs | ≈ tokens | vs. 6.9k today |
|---|---|---|---|
| **Writing** Satsuma (codegen, conversion, authoring) | EBNF, conventions, common mistakes, examples, the generate-workflow steps | ~3,600 | **~48% cut** |
| **Reading** Satsuma (lineage, impact, coverage, audit) | Command reference, composition guidance, transform classification, the `@ref` and path-syntax parts of conventions, the read-workflow steps | ~3,300 | **~52% cut** |
...
resident-vs-loaded distinction made explicit for each envelope. Every ≈-token
figure in this PRD is a bytes/4 estimate and must be replaced by a measured
number.
...
- **No MCP server.** Measurement is expected to show eagerly-injected tool
  schemas are the *worst* option on the resident-cost axis (~23 commands × a few
  hundred tokens, paid whether used or not). It is measured as a comparison
  point, not built.

## Acceptance criteria

- [ ] Canonical section files exist under `reference/`, containing the same
      guidance as today's `AI-AGENT-REFERENCE.md` — no content rewritten.
- [ ] `prebuild.js` bakes a **section map** rather than one string, and every
      envelope is composed from it with no hand-written content.
- [ ] `satsuma agent-reference` with no flags produces byte-identical output to
      the current command, verified by a test comparing it against the
      concatenation of all sections.
- [ ] `--section <name>`, `--profile write|read`, and `--list` implemented, with
...
- [ ] **Measured** per-tokenizer token counts for: the whole document, each
      section, each profile, and each envelope's *resident* vs *loaded* cost —
      replacing every bytes/4 estimate in this PRD.
- [ ] MCP tool-schema resident cost measured as a comparison point (schemas
