...
/**
 * Closed map of URL route → file name under CLIENT_DIR. Static client assets
 * are served only through this map, so request paths are never joined onto
 * the filesystem and cannot traverse outside the client directory.
 */
const CLIENT_ROUTES: Record<string, string> = {
  "/index.html": "index.html",
  "/app.js": "app.js",
  "/app.js.map": "app.js.map", // useful during local debugging
  "/satsuma-viz.js": "satsuma-viz.js",
  "/examples.json": "examples.json",
  "/satsuma-logo.png": "satsuma-logo.png",
  "/lexend-latin.woff2": "lexend-latin.woff2",
};
...
function makeHandler(
  ...
  return (req, res) => {
    ...
    // Client assets under CLIENT_DIR, keyed by exact route. Serving only from
    // this closed map (never by joining the request path onto the directory)
    // is what rules out path traversal — new assets must be added here.
    ...
    const clientFile = CLIENT_ROUTES[rawPath];
    if (clientFile) {
      const served = serveStaticFile(res, path.join(CLIENT_DIR, clientFile));
