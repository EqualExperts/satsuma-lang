...
test("generation recreates both ignored build-version modules from a clean checkout", (t) => {
  // Type-aware lint starts without ignored build outputs in CI. Its prerequisite
  // must recreate every imported module without requiring a package build.
  const repoRoot = createFixtureRepo(t);
  const buildVersion = "0.11.0-dev.abc1234";

  const generatedPaths = generateBuildVersionModules(repoRoot, buildVersion);

  assert.deepEqual(generatedPaths, Object.values(BUILD_VERSION_MODULE_PATHS));
  for (const relativePath of generatedPaths) {
    const source = fs.readFileSync(path.join(repoRoot, relativePath), "utf8");
    assert.match(source, /Auto-generated by scripts\/release-metadata\.mjs/);
    assert.match(source, /export const BUILD_VERSION = "0\.11\.0-dev\.abc1234";/);
  }
});

test("root JavaScript lint declares build-version generation as its prerequisite", () => {
  // Generation logic alone is insufficient if the CI lint entry point can
  // bypass it; this guards the npm lifecycle wiring that clean checkouts use.
  const scriptsDir = path.dirname(fileURLToPath(import.meta.url));
  const rootPackage = readJson(path.join(scriptsDir, "..", "package.json"));

  assert.equal(
    rootPackage.scripts["prelint:js"],
    "node scripts/release-metadata.mjs generate-build-versions",
  );
});
