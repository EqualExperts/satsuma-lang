...
/** Generated source modules consumed by the two version-bearing executables. */
export const BUILD_VERSION_MODULE_PATHS = Object.freeze({
  cli: "tooling/satsuma-cli/src/generated/build-version.ts",
  lsp: "tooling/satsuma-lsp/src/generated/build-version.ts",
});

const BUILD_VERSION_MODULE_DESCRIPTIONS = Object.freeze({
  cli: "Version baked into this CLI build.",
  lsp: "Version advertised by this language-server build.",
});
...
/**
 * Generates the ignored TypeScript modules that carry one resolved build identity.
 *
 * Callers may select one consumer during a package prebuild; clean-checkout lint
 * generates both so type-aware analysis never depends on artifacts from an
 * earlier build. Returns repo-relative paths for verification and logging.
 */
export function generateBuildVersionModules(
  repoRoot,
  buildVersion,
  consumers = Object.keys(BUILD_VERSION_MODULE_PATHS),
) {
  assertVersion(buildVersion);
  const generatedPaths = [];

  for (const consumer of consumers) {
    const relativePath = BUILD_VERSION_MODULE_PATHS[consumer];
    const description = BUILD_VERSION_MODULE_DESCRIPTIONS[consumer];
    if (!relativePath || !description) {
      throw new Error(`Unknown build-version consumer: ${consumer}`);
    }
    ...
    fs.writeFileSync(
      outputPath,
      `// Auto-generated by scripts/release-metadata.mjs — do not edit manually.\n` +
...
function printUsage() {
  console.error(
    "Usage: node scripts/release-metadata.mjs " +
      ...
      "generate-build-versions [cli|lsp] | " +
...
function runCli() {
  ...
  if (command === "generate-build-versions") {
    if (value && !Object.hasOwn(BUILD_VERSION_MODULE_PATHS, value)) {
      printUsage();
      process.exitCode = 1;
      return;
    }
    const buildVersion = resolveBuildVersion(repoRoot);
    const consumers = value ? [value] : undefined;
    const generatedPaths = generateBuildVersionModules(repoRoot, buildVersion, consumers);
    console.log(`Generated ${generatedPaths.length} build-version module(s) for ${buildVersion}.`);
    return;
  }
  if (command === "bump") {
    if (!value) {
